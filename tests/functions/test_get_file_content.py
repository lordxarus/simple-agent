...
class TestGetFileContent(unittest.TestCase):
    ...
    def test_no_read_permission(self):
        with TemporaryDirectory() as tmpdir:
            ...
            assert Err.NO_PERMISSION_FS(main_py) in get_file_content(
                tmpdir, main_py.as_posix()
            )
    ...
    def test_truncation(self):
        txt_p = Path(__file__).parent / Path(
            "./test_get_file_content_truncation_sample.txt"
        )
        assert (
            f"test_get_file_content_truncation_sample.txt truncated to {config.gfc.N_CHAR} characters"
            in get_file_content(".", txt_p)
        )
